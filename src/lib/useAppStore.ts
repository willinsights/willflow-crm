...
export function useAppStore() {
  ...
  const filteredProjects = projects.filter(project => {
    ...
    if (searchQuery) {
      ...
      const matchesClient = clients.find(c => c.id === project.clientId)?.name?.toLowerCase().includes(query);
  ...
  const projectsByPhase = {
    captacao: projects.filter(p => p.phase === 'captacao'),
    edicao: projects.filter(p => p.phase === 'edicao'),
    finalizados: projects.filter(p => p.phase === 'finalizados')
  };
