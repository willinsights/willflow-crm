...
interface LoginPageProps {
  onLogin: (email: string, password: string) => Promise<{ success: boolean; error?: string; mustChangePassword?: boolean }>;
}
...
export default function LoginPage({ onLogin }: LoginPageProps) {
  ...
  const fillDemoCredentials = (type: 'admin' | 'editor' | 'freelancer') => {
    switch (type) {
      case 'admin':
        setEmail('geral@in-sights.pt');
        setPassword('Insights26@');
        break;
      case 'editor':
      ...
      <div className="w-full max-w-md relative">
        <div className="glass-card p-6 md:p-8 shadow-glow">
          ...
          <form onSubmit={handleSubmit} className="space-y-3 md:space-y-4">
            ...
            {/* Error Message */}
            {error && (
              <div className="p-3 rounded-lg bg-red-500/10 border border-red-500/30 text-red-400 text-xs md:text-sm flex items-center gap-2">
          ...
          <div className="mt-6 md:mt-8 pt-4 md:pt-6 border-t border-white/10">
            ...
            <p className="text-xs text-muted-foreground text-center mt-3 opacity-70">
              Ou utilize credenciais criadas em Colaboradores
            </p>
          </div>
